...
class DieselGeneratorSimulator:
    ...
    def generate_output(self, df: pd.DataFrame, power_request_series: pd.Series) -> Dict[str, Any]:
        ...
        running_output = np.zeros(num_steps, dtype=bool)
        power_output = np.zeros(num_steps)
        frequency_output = np.zeros(num_steps)
        temperature_output = np.zeros(num_steps)
        fuel_level_output = np.zeros(num_steps)
        runtime_output = np.zeros(num_steps)
        needs_maintenance_output = np.zeros(num_steps, dtype=bool)
        ...
        for i in range(num_steps):
            ...
            if self.running and self.fuel_level > 0:
                # Update runtime
                self.runtime_hours += 1 # Assuming 1 hour time step
            ...
            # Store results
            running_output[i] = self.running
            power_output[i] = output_power
            frequency_output[i] = frequency if self.running else 0
            temperature_output[i] = self.temperature
            fuel_level_output[i] = self.fuel_level
