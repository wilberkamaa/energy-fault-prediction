...
class FaultInjectionSystem:
    ...
    def check_fault_conditions(self, system_state: Dict[str, np.ndarray], 
        ...
        # Inverter faults
        if system_state.get('inverter_temperature') is not None:
            if system_state['inverter_temperature'][hour] > fault_config['thresholds'].get('inverter_temperature', 80):
                potential_faults.append(
                    (FaultType.INVERTER_IGBT_FAILURE,
                     ...
                     (system_state['inverter_temperature'][hour] - fault_config['thresholds'].get('inverter_temperature', 80)) / 10)
                )
